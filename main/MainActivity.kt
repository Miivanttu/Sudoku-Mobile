...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(s: Bundle?) {
        ...
        }while(!countSolutions(outputArray))
        ...
        repeat(numToRemove) {indeks ->
            ...
            while (true) {
                do {
                    ...
                    println("check $indeks")
                } while (outputArray[row][col] == 0)
                temp = outputArray[row][col]
                outputArray[row][col] = 0
                println("change $indeks $row;$col")
                println(outputArray[row][col])
                if (countSolutions(inputArray)) {
                    break
                }
                outputArray[row][col] = temp
            }
        }
        return outputArray
    }

    private fun countSolutions(board: Array<IntArray>): Boolean {
        val solutions = mutableListOf<Array<IntArray>>()
        solveSudokuTest(board, solutions)
        println("Solutions now : " + solutions.size)
